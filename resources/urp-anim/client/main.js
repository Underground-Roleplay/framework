...
alt.everyTick(() => {
    if (
        !natives.isEntityDead(alt.Player.local.scriptID, true) &&
        !natives.isPedInAnyVehicle(alt.Player.local.scriptID, true)
    ) {
        ...
        if (!natives.isPauseMenuActive()) {
            if (natives.isDisabledControlJustPressed(0, 36)) {
                ...
                if (crouch) {
                    natives.resetPedMovementClipset(
            ...
            if (!point) {
                ...
                pointInterval = alt.everyTick(() => {
                    ...
                    natives.setTaskMoveNetworkSignalBool(
                        alt.Player.local.scriptID,
                        'isFirstPerson',
                        natives._0xEE778F8C7E1142E2(
                            natives._0x19CAFA3C87F7C2FF()
                        ) === 4
                    );
                });
            }
...
const playAnim = async (data) => {
    ...
    if (animations[data]) {
        if (animations[data].dict) await loadAnim(animations[data].dict);
        if (
            animations[data].type == 'normal' &&
            !natives.isPedInAnyVehicle(scriptID, true)
        ) {
            natives.taskPlayAnim(
                scriptID,
                animations[data].dict,
                animations[data].anim,
                3.0,
                -3.0,
                -1,
                animations[data].flag,
                0,
                0,
                0,
                0
            ...
            animations[data].type == 'scenario' &&
            !natives.isPedInAnyVehicle(scriptID, true)
        ) {
            let pedpos = natives.getEntityCoords(alt.Player.local, true);
            natives.taskStartScenarioAtPosition(
                scripalt.Player.localtID,
                animations[data].dict,
                pedpos.x,
                pedpos.y,
                pedpos.z - 1,
                natives.getEntityHeading(alt.Player.local),
                0,
                0,
                false
            );
            ...
            if (alt.Player.local.animObject) {
                ...
                natives.setEntityAsMissionEntity(
                    alt.Player.local.animObject,
                    false,
                    false
                );
                natives.networkFadeOutEntity(
                    alt.Player.local.animObject,
                    false
                );

                while (
                    natives.networkIsEntityFading(alt.Player.local.animObject)
                ) {
                    alt.setTimeout(() => {}, 100);
                }
            ...
            alt.Player.local.animObject = natives.createObject(
                natives.getHashKey(animations[data].prop),
                coords.x,
                coords.y,
                coords.z,
                true,
            ...
            if (animations[data].height) {
                natives.attachEntityToEntity(
                    alt.Player.local.animObject,
                    scriptID,
                    natives.getPedBoneIndex(scriptID, animations[data].bone),
                    animations[data].height,
                    animations[data].pos1,
                    animations[data].pos2,
                    animations[data].pos3,
                    animations[data].pos4,
                    animations[data].pos5,
                    true,
                    false,
                    false,
                    true,
