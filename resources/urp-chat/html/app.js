...
function colorify(text) {
    ...
    if (matches.length > 0) {
        ...
        for (let i = matches.length - 1; i >= 0; --i) {
            let color = matches[i].found.substring(
                1,
                matches[i].found.length - 1
            );
            let insertHtml =
...
function scrollTo(el, to, time) {
    ...
    var tick = function () {
        el.scrollTop =
            (neg ? -el.scrollTop : el.scrollTop) +
            diff * ((new Date() - last) / time);
        if (el.scrollTop == prevVal) return;
...
function checkOverflow() {
    if (
        ...
        document.querySelector('.msglist').clientHeight
    ) {
        if (
            !document.querySelector('.msglist').classList.contains('overflowed')
        )
...
function openChat(insertSlash) {
    ...
    if (!chatOpened) {
        ...
        if (insertSlash) msgInputLine.value = '/';
        msgInputLine.focus();
        chatOpened = true;
    } else {
        return false;
...
function highlightChat() {
    scrollTo(
        ...
        document.querySelector('.msglist').scrollHeight,
        0
    );
