    ...
    if (isAllowed) {
        ...
        if (identifier.match(ssnRegex)) {
            const target = alt.Player.all.find(
                (source) => source.playerData.ssn === identifier
            );
            if (!target)
                return alt.emitClient(
                    ...
                    Core.Translate('SYSTEM.NO_TARGET_FOUND')
                );
            ...
            const target = alt.Player.all.find(
                (source) => source.playerData.id === identifier
            );
            if (!target)
                return alt.emitClient(
                    source,
...
chat.registerCmd('dv', (source) => {
    ...
    if (isAllowed) {
        const closestVeh = alt.Vehicle.all.find(
            (v) => source.pos.distanceTo(v.pos) < 50
        );
        if (!closestVeh) {
            alt.emitClient(
                source,
    ...
    if (isAllowed) {
        if (!socialID || !reason) {
            alt.emitClient(
                ...
                'error',
                Core.Translate('COMMANDS.LABEL'),
                'Type /ban social ID reason'
            );
            return;
        }
        const target = alt.Player.all.find(
            (source) => source.playerData.socialID === socialID
        );
...
chat.registerCmd('cloth', (source, [component, drawable, texture]) => {
    if (!component || !drawable || !texture) {
        alt.emitClient(
            ...
            'error',
            Core.Translate('COMMANDS.LABEL'),
            'use /cloth [component drawable texture]'
        );
        return;
    }
...
chat.registerCmd('createInteraction', (source) => {
    Core.Interactions.createSingleInteraction(
        source.pos,
        'random',
        'test',
        false,
...
chat.registerCmd('createInteractionWithMarker', (source) => {
    Core.Interactions.createSingleInteraction(
        ...
        'test',
        false,
        { name: 'Random event marker' },
        undefined,
        { type: 5, color: new alt.RGBA(0, 181, 204, 200) }
    );
});
...
chat.registerCmd('createInteractionWithPed', (source) => {
    Core.Interactions.createSingleInteraction(
        ...
        false,
        undefined,
        { pedType: 2, modelHash: alt.hash('s_m_o_busker_01') }
    );
});

chat.registerCmd('createPed', (source) => {
    Core.Entities.createPed(source.pos, 0, {
        pedType: 3,
...
chat.registerCmd('createMarker', (source) => {
    ...
    Core.Entities.createMarker(source.pos, 0, {
        type: 0,
        r: color.r,
        g: color.g,
        b: color.b,
...
chat.registerCmd('god', (source) => {
    ...
    alt.emitClient(
        ...
        'error',
        Core.Translate('SYSTEM.LABEL'),
        `${source.invincible ? 'GOD MODE ON' : 'GODE MODE OFF'}`
    );
});
...
chat.registerCmd('toggleEngine', (source) => {
    if (!source.vehicle) {
        alt.emitClient(
            ...
            Core.Translate('VEHICLES.NOT_IN_A_VEH')
        );
        return;
    }
...
chat.registerCmd('kick', (source, [identifier, reason]) => {
    if (!identifier) {
        alt.emitClient(
            ...
            'error',
            Core.Translate('SYSTEM.LABEL'),
            Core.Translate('SYSTEM.TARGET_NEEDED_IDENTIFIER')
        );
        return;
    }
    const isAllowed = Core.Functions.hasPermission(source, 'admin');
