...
const setDuty = (source) => {
    if(source.playerData.job.onDuty){
        ...
        alt.emitClient(source,'notify', 'success', Core.Translate('JOB.LABEL', { jobname: source.playerData.job.name.toUpperCase() }), Core.Translate('JOB.JOB_ENDED'))
    } else {
        source.playerData.job.onDuty = true
        alt.emitClient(source,'notify', 'success', Core.Translate('JOB.LABEL', { jobname: source.playerData.job.name.toUpperCase() }), Core.Translate('JOB.JOB_STARTED'))
    }
    saveJobdata(source)
    Core.Functions.emitPlayerData(source, 'job', source.playerData.job)
}
