...
function OpenConceMap(data) {
    $('#viwer-main').empty().append(`
        <div class='between c-c'><h3 class="mb-3">${capitalize(
            data
        )}</h3><i class="fal fa-times" onclick="Close()"></i></div>
    ...
    ConceEstoq.map((veh) => {
        if (veh.category == data) {
            ...
            <div class="card-car mb-2 ms-2" >
                <div class="card-car-internal" onclick="setData('${
                    veh.model
                }')">
                    <div class="card-car-img" style="background: url('./img/${
                        veh.model
                    }.png')" >
                    </div>
                    <div class="card-car-info">
                        <label class="full between mb-4">
                            ...
                            <div class="c-c"><p>Stock</p><p class="ms-2">${
                                veh.stock
                            }</p></div>
                        </label>
                        <label class="full between">
                            <div class="c-c"><i class="fal fa-sack-dollar"></i><p class="ms-2">${FormatNumber(
                                veh.price
                            )},00</p></div>
...
function buy() {
    if (LastVeh != null) {
        ...
        document.getElementById(
            'modal-body'
        ).innerHTML = `you are sure you want to buy a ${capitalize(
            LastVeh.model
        )} for $ ${LastVeh.price}`;
    }
}
