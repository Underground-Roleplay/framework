...
function PriceSelect(type, price) {
    ...
    switch (type) {
        case 'diesel':
            ...
            $('.card-combustivel-oil').css(
                'background-color',
                'rgba(0, 0, 0, 0.432)'
            );
            break;
        case 'gasoline':
            $('.card-combustivel').css(
                'background-color',
                'rgba(255, 167, 4, 0.302)'
            );
        ...
        if (wallet > 0) {
            if (value >= wallet || value / priceSelect >= tankFull - tank) {
                if (value / priceSelect >= tankFull - tank) {
                    ...
                    $('#price').val(wallet);
                    $('#liters').val(
                        parseFloat(wallet / priceSelect).toFixed(2)
                    );
                }
            } else {
...
function attPrices() {
    ...
    if (value > 0) {
        ...
        document.getElementById('taxe').innerHTML = `$ ${parseFloat(
            total * porcentagem
        ).toFixed(2)}`;
        document.getElementById('total').innerHTML = `$  ${total}`;
    }
    document.getElementById('taxeP').innerHTML = `% ${parseFloat(
        porcentagem * 100
    ).toFixed(0)}`;
    document.getElementById('priceSelect').innerHTML = `$ ${parseFloat(
        priceSelect
    ).toFixed(2)}`;
}
