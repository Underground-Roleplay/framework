...
const updateDrag = () => {
    ...
    $('.slot-item-img').draggable({
        ...
        start: function (event, ui) {
            ...
            if (itemData.name === undefined || itemData.type === undefined)
                return;
            $('.actions').show();
        },
        stop: function () {
    ...
    $('.active-slot-I').droppable({
        ...
        drop: function (event, ui) {
            itemData = {
                name: ui.draggable.data('name'),
                type: ui.draggable.data('type'),
                origin: ui.draggable.data('origin'),
            };
            if (itemData.origin === undefined || itemData.name === undefined)
                return;
...
const updateDragChest = () => {
    ...
    $('.chest-drop').droppable({
        ...
        drop: function (event, ui) {
            itemData = {
                name: ui.draggable.data('name'),
                oringin: ui.draggable.data('origin'),
                amount: ui.draggable.data('amount'),
            };
            if (itemData.name === undefined || itemData.oringin === 'chest')
                return;
            alt.emit(
                'inventory:transferChest',
                itemData.name,
                itemData.amount,
                chestType
            );
    ...
    $('.inventory-drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.name === undefined || itemData.oringin === 'inventory')
                return;
            ...
            if (itemData.oringin === 'activedSlot') {
                alt.emit(
                    'inventory:transfer:activedInventory',
                    itemData.name,
                    itemData.slot
                );
            }
...
const createItemInventory = (obj, amount) => {
    ...
    <div class="slot-item">   
      ...
      <img class="slot-item-img" data-amount="${amount}" data-type="${
        obj.type
    }" data-name="${obj.name}" data-use="${
        obj.useable
    }" data-origin="${'inventory'}"  width="64px" da height="64px" src="./img/images/${
        obj.image
    }">
...
const createItemChest = (obj, amount) => {
    ...
    <div class="slot-item">   
      <p class="amount">x${amount}</p>
      <img class="slot-chest-img" data-amount="${amount}" data-type="${
        obj.type
    }" data-name="${obj.name}" data-use="${
        obj.useable
    }" data-origin="${'chest'}"  width="64px" da height="64px" src="./img/images/${
        obj.image
    }">
      <p class="name">${obj.label}</p>                
    </div> 
...
const updateSlotActived = (actived) => {
    for (let i = 0; i < actived.length; i++) {
        if (
            actived[i].name !== '' &&
            actived[i].name !== null &&
            Object.keys(actived[i]).length > 0
        ) {
            $(`#activedSlot-${i}`).empty().append(` 
        <img data-name="${
            actived[i].name
        }" data-origin="${'activedSlot'}" data-slot="${i}"   class="slotActived-img-I" width="65px" height="65px" src="./img/images/${
                actived[i].name
            }.png" alt="">
