...
function FormatAttachmentInfo(data) {
    // We need an event with callback here D: 
    // $.post(
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        alt.emit("inventory:Notify", "Attachments are unavailable for this gun.", "error")
        // $.post(
...
function handleDragDrop() {
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            alt.emit('inventory:dropItem', fromInventory, fromData, parseInt(amount))
            // $.post(
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        alt.emit('inventory:combineWithAnim', combineslotData.toData.combinable, combineslotData.toData.name, combineslotData.fromData.name)
        // $.post(
        ...
        alt.emit('inventory:combineItem', combineslotData.toData.combinable.reward, combineslotData.toData.name, combineslotData.fromData.name)
        // $.post(
...
function optionSwitch(
    ...
    alt.emit('inventory:setInventoryData', $fromInv.attr("data-inventory"), $toInv.attr("data-inventory"), $fromSlot, 
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            alt.emit('inventory:playDropSound')
            alt.emit('inventory:setInventoryData', $fromInv.attr("data-inventory"), $toInv.attr("data-inventory"), $fromSlot, 
            $toSlot, $toAmount)
...
function InventoryError($elinv, $elslot) {
    ...
    alt.emit('inventory:playDropFail')
...
(() => {
    ...
    window.onload = function(e) {
        if('alt' in window){
            alt.on('inventory:open', Inventory.Open)
            alt.on('inventory:close', Inventory.Close)
            alt.on('inventory:update', Inventory.Update)
        }
...
$(document).on("click", "#rob-money", function(e) {
    ...
    alt.emit('inventory:robMoney', TargetId)
    // $.post(
...
$("#item-give").droppable({
    ...
    drop: function(event, ui) {
        ...
        alt.emit('inventory:giveItem', fromInventory, fromData, parseInt(amount))
        // $.post(
