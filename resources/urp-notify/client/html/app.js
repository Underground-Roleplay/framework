...
function showNotify(type, tittle, message,time)  {
		...
		if (type == "error"){
			...
			$(html).appendTo(".notification-container").animate({	opacity: 1 }, 'fast').delay(time).fadeOut(2000);
			setTimeout(() => {
				$(".fill").animate(
					{
						clip: clippings[1]
...
function showNotifyCenter(type, message,time)  {
	...
	if (type == "comunicado"){
		...
		$(html).appendTo(".notification-container-topcenter").animate({	opacity: 1 }, 'fast').delay(time).fadeOut(2000);
