...
const openCellPhone = () => {
    Core.Browser.open(
        'http://resources/urp-phone/client/html/index.html#/',
        true,
        false
    );
...
const openCallLink = (number) => {
    Core.Browser.open(
        `http://resources/urp-phone/client/html/index.html#/PhoneCallfull/CallLink?number=${number}&recebendo=true`,
        true,
        false
    );
...
alt.onServer(
    'Phone:GetAllData',
    (DataContact, DataRecent, DataHistory, DataNumber, DataSsn) => {
        Core.Browser.emit(
            'Phone:GetAllData',
            DataContact,
            DataRecent,
