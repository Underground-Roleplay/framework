...
//>  GET DATA IN LOAD INITIAL PHONE
    const GetAllData = async(source) => {
        ...
        const contact = await executeSync('SELECT contact from phone_contact WHERE ssn = ?', [Core.Functions.GetSsn(source)])
        const recent = await executeSync('SELECT recent from phone_recent WHERE ssn = ?', [Core.Functions.GetSsn(source)])
        const history = await executeSync('SELECT * from chat WHERE id_player_one = :myphone OR id_player_two = :myphone ', {myphone: Core.Functions.GetNumber(source)}) 
        const mynumber = Core.Functions.GetNumber(source) ;
        const ssn = Core.Functions.GetSsn(source);
        
        alt.emitClient(source, 'Phone:GetAllData', contact,recent,history,mynumber,ssn)
    }
...
//>  VERIFY THE EXISTENCE IF IT DOESN'T EXIST THE CHAT IS CREATED
    const GetChatId = async(source,phone) => {
        ...
        ServLog(phone) 
        const result = await executeSync('SELECT * from chat WHERE id_player_one = :myphone AND id_player_two = :phone  OR  id_player_one = :phone AND id_player_two = :myphone', {myphone:Core.Functions.GetNumber(source),phone:phone})
        if (result.length >= 1) {
            alt.emitClient(source, 'Phone:GetIdChat',result[0].id)   
            ServLog(`CHAT FOUND ID: ${result[0].id}`) 
        }else{
            const save= await insertSync('INSERT INTO chat (id_player_one,id_player_two) VALUES (?,?)', [Core.Functions.GetNumber(source), phone])
    ...
    const UpdateContain = async(source,phone)=>{
        ...
        if (result[0].contain_message === 0) {
            await updateSync('UPDATE chat SET contain_message = ? WHERE id = ?', [1, result[0].id], undefined, alt.resourceName)
        }
    }
...
//  SAVE  THE  MESSAGE 
    const SaveMessageId = async(source,type,message, chat_id,phone) => {
        ...
        await insertSync('INSERT INTO chat_message (type,message,ssn,chave_id) VALUES (?,?,?,?)', [type,message, Core.Functions.GetSsn(source),chat_id])
        await updateSync('UPDATE chat SET last_message = ? WHERE id = ?', [message,chat_id], undefined, alt.resourceName)
        Core.Functions.PhoneTunel(source,'Phone:ReciveMessage',{type:type,message:message,ssn:Core.Functions.GetSsn(source),chat_id:chat_id},phone);
    }
    alt.onClient('Phone:CreateMessage', (source,type,menssage,chat_id,phone) => {
...
//>  GET ALL CONVERSATION
    const GetChatMessage = async(source, chat_id) => {
        ...
        const result = await executeSync('SELECT * from chat_message WHERE chave_id = ? ', [chat_id])
...
//>
    const UpdateListContact = async( ssn, data) => {
        await updateSync('UPDATE phone_contact SET contact = ? WHERE  ssn = ?', [JSON.stringify(data), ssn], undefined, alt.resourceName)
    }
...
//>
    const UpdateListRecents = async(ssn, data) => {
        await updateSync('UPDATE phone_recent SET recent = ? WHERE ssn = ?', [JSON.stringify(data), ssn], undefined, alt.resourceName)
    }
...
//>
    const Visualized = async(source, chat_id) => {
        await updateSync('UPDATE phone_message SET visualized = ? WHERE chat_id = ?', [1, chat_id], undefined, alt.resourceName)
    }
...
//>
    const Deleted = async(source, id) => {
        await updateSync('UPDATE phone_message SET deleted = ? WHERE id = ?', [1, id], undefined, alt.resourceName)
    }
