...
const openSkinshop = (data) => {
    ...
    Core.Browser.open(
        'http://resources/urp-skinshop/client/html/ui.html',
        true,
        false
    );
    ...
    Core.Browser.on('load', () => {
        Core.Browser.emit(
            'Skinshop:updateSex',
            alt.Player.local.model === alt.hash('mp_m_freemode_01') ? 0 : 1
        );
    });
    ...
    for (let index = 0; index < Aroupa.length; index++) {
        if (Aroupa) {
            alt.emitServer(
                'Skinshop:Closenobuy',
                index,
                Aroupa[index].drawable,
                Aroupa[index].texture
            );
        }
    }
}
...
function CamPos(txt) {
    charCreation.map((cam) => {
        if (cam.name == txt) {
            Core.Utils.createCamera(
                alt.Player.local.pos.x + cam.x,
                alt.Player.local.pos.y + cam.y,
                alt.Player.local.pos.z + cam.z,
                cam.rx,
                cam.ry,
                cam.rz,
                cam.timelapse
