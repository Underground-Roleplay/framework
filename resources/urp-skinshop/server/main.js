...
alt.onClient('Skinshop:Buy', (source, i, component, color,type) => {
    if (Core.Money.hasFullMoney(source, 250)) {
        source.setClothes(i, component, color)
        if (type === 'prop') {
            if (i === 6) return Core.Character.changeProps(source, 3, parseInt(component),parseInt(color) )
            if (i === 7)  return Core.Character.changeProps(source, 4, parseInt(component),parseInt(color) )
            Core.Character.changeProps(source, i, parseInt(component),parseInt(color) )
        }else{
