...
const createVoiceChannel = (name, range, spatial = true) => {
    if (!name || channels[name])
        throw new Error(
            'You need to add a name to your voice channel, and he needs to be unique'
        );
    try {
        ...
        if (e.message === 'Failed to create base object') {
            alt.logWarning(
                'The alt:V voice chat is not enabled and this resource will cease to work. To enable it, specify the "voice" entry in the server config.'
            );
        } else {
            throw e;
        }
...
const addSourceToChannel = (source, channelName) => {
    if (!channelName || !channels[channelName] || !source)
        throw new Error('You need to specify a valid channel/source');
...
const toggleVoiceChannel = (index, source) => {
    ...
    chat.send(
        source,
        '{80eb34}Voice Distance changed to {34dfeb}' +
            config.rangeArray[index] +
            '{80eb34}m.'
    );
};
